 ...
 * ntersection at all, return null.
...
(2nd node in A and 3rd node in B) are different node references. In other words, they point to 
two different locations in memory, while the nodes with value 8 in A and B (3rd node in A and 4th node 
...
var getIntersectionNode = function (headA, headB) {
  ...
  if (countA > countB) {
    while (count < (countA - countB) && currentA != null) {
      count++;
      currentA = currentA.next;
    }
  } else if (countA < countB) {
    while (count < (countB - countA) && currentB != null) {
      count++;
      currentB = currentB.next;
    }
  }
